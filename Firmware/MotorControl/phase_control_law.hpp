...
public:
     ...
     */
    virtual ZfocIntf::MotorIntf::Error on_measurement(
     ...
     *           be truly active.
     */
    virtual ZfocIntf::MotorIntf::Error get_output(
...
private:
    ZfocIntf::MotorIntf::Error on_measurement(
            std::optional<float> vbus_voltage,
            std::optional<std::array<float, 3>> currents,
            uint32_t input_timestamp) final;

    ZfocIntf::MotorIntf::Error get_output(
            uint32_t output_timestamp,
            float (&pwm_timings)[3],
...
protected:
    ...
    virtual ZfocIntf::MotorIntf::Error get_alpha_beta_output(
            uint32_t output_timestamp,
